...
export class MockRuntime extends EventEmitter {
  ...
  public async start(
    ...
    const rom = new ROM(
      await this.fileAccessor.readFile(this.normalizePathAndCasing(program)),
      0x4000
    );
    this._system.loadRom(rom);
    this._system.reset();
  ...
  private findNextStatement(reverse: boolean, stepEvent?: string): boolean {
    // perhaps find location in asm files here?
    if (stepEvent) {
